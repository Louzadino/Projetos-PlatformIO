...
class Jogo:
    ...
    def visaoJogar(self):
        camera = openCv.VideoCapture(0, openCv.CAP_V4L2)
        
        # Força o formato de compressão MJPG (resolve o gargalo da USB)
        camera.set(openCv.CAP_PROP_FOURCC, openCv.VideoWriter_fourcc('M', 'J', 'P', 'G'))

        # Define a resolução
        camera.set(3,640)
        camera.set(4,480)

        # Limita o buffer para pegar sempre o frame mais recente
        camera.set(openCv.CAP_PROP_BUFFERSIZE, 1)

        # Dá o tempo necessário para o hardware ligar
        time.sleep(2)
        ...
        while True:
            ...
            if not success or imagem is None:
                print("Frame vazio ou falha na leitura. Tentando novamente...")
                continue # Ignora o erro, pula o resto do bloco e tenta capturar o próximo frame
